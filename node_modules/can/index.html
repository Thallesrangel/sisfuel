...
<head>
	<meta charset="utf-8">
	<title>CanJS — Build CRUD apps in fewer lines of code.</title>
	<meta name="description" content="CanJS is a JavaScript framework for building CRUD apps in fewer lines of code.">
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
	<meta property="og:image" content="https://www.bitovi.com/hubfs/open-source/os-canjs.png">
	<meta property="og:description" content="CanJS is a JavaScript framework for building CRUD apps in fewer lines of code.">
	<meta property="og:title" content="CanJS — Build CRUD apps in fewer lines of code.">
	<script type="application/ld+json">
		{
			...
			"name": "CanJS — Build CRUD apps in fewer lines of code.",
	...
	<body>
		<input type="checkbox" id="nav-trigger" class="nav-trigger"/>
	  	<div data-current-page="canjs" id="everything">
  <div class="header">
...
<ul class="social-side">
  ...
  <section class="title">
	<div class="page-type">
		<h1>CanJS — Build CRUD apps in fewer lines of code.</h1>
			<ul class="title-social">
				
				<li>
					<a class="npm-button" href="https://www.npmjs.com/package/can">
						<img src="https://img.shields.io/badge/npm%20package-6.4.0-brightgreen.svg" alt="npm package badge" />
					</a>
				</li>
				<li>
    ...
    <p>CanJS is a JavaScript framework for building CRUD apps in fewer lines of code.</p>
  ...
  <div class="col-container">
    <div class="content">
      <div class="content-details">
        ...
        <p>CanJS’s <a href="doc/can-stache.html" title="Live binding templates.">stache templating language</a> can directly read the state and values from <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise">Promises</a>.</p>
        <p>
...
<pre><code class="language-html">{{# if(this.promise.isPending) }}
  Loading…
{{/ if }}
...
<p>With a single line of code, CanJS creates a model that represents the objects returned by a backend API.
See how <code>Todo</code> is created by passing a URL to <a href="doc/can-realtime-rest-model.html" title="Connect a type to a restful data source and automatically manage lists.">realtimeRestModel()</a>.</p>
<p>The model layer is responsible for making GET, POST, PUT, and DELETE requests to your backend.
With your component UI code using the model’s standard interface to make requests, if the backend API changes,
you only have to configure the model and not change every component that uses that backend API.</p>
<p>By default, CanJS assumes your backend API is RESTful. If your backend API isn’t RESTful, that’s ok!
CanJS has configuration options for you to control how it makes requests, parses data, and more.</p>
</div>
<div class="right-col">
<pre><code class="language-js">import { realtimeRestModel } from &quot;can&quot;;
...
<p>CanJS’s <a href="doc/can-stache.html" title="Live binding templates.">stache templating language</a> is similar to Handlebars and Mustache.
Wherever you see <code>{{ }}</code> in a template, CanJS evaluates the expression inside to either
print a value or perform some basic logic, like <a href="doc/can-stache.helpers.if.html" title="can-stache.helpers.if">#if</a> and
<a href="doc/can-stache.helpers.for-of.html" title="Loop through a list of values, keys in an object, or integers.">#for(of)</a>.</p>
<p>Stache is able to read the state and value of Promises. See <code>isPending</code>, <code>isRejected</code>,
and <code>isResolved</code> being read on <code>this.todosPromise</code> in the example code? Those return
true depending on the current state of the Promise. <code>reason</code> is provided if the
Promise is rejected with an error, and <code>value</code> contains the resolved value if
the promise succeeds.</p>
<p>These helpers make it much easier to include loading and error states in your app.
We promise you’ll love writing your templates this way.</p>
</div>
<div class="right-col">
<pre><code class="language-html">{{# if(this.todosPromise.isPending) }}
    Loading todos…
{{/ if }}
...
<p>Here you can see CanJS’s model layer in action. When <code>Todo.getList({sort: &quot;name&quot;})</code> is called,
CanJS makes a GET request to <code>/api/todos?sort=name</code></p>
<p>When the array of to-dos comes back, CanJS associates that array with the query <code>{sort: &quot;name&quot;}</code>.
When new to-dos are created, they’re added to the list that’s returned <em>automatically</em>, and
in the right spot! You don’t have to write any code to make sure the new to-do gets inserted
into the right spot in the list.</p>
<p>CanJS does this for filtering as well. If you make a query with a filter (e.g. <code>{filter: {complete: true}}</code>),
when items are added, edited, or deleted that match that filter, those lists will be updated automatically.</p>
<p>Save yourself time by not writing code that updates your app’s UI.</p>
</div>
<div class="right-col">
<pre><code class="language-js">import { realtimeRestModel } from &quot;can&quot;;
...
<p>Below is an entire app that shows off some of the best features of CanJS:</p>
<ul>
<li>One line of code to create a model from the data returned by a backend API (with <a href="doc/can-realtime-rest-model.html" title="Connect a type to a restful data source and automatically manage lists.">realtimeRestModel</a>).</li>
<li><code>isPending</code>, <code>isRejected</code>, <code>isResolved</code>, and <code>value</code> helpers for directly reading the state of a Promise.</li>
<li>When you add a to-do, it automatically gets inserted into the list in the right position.</li>
</ul>
